...
\begin{enumerate}
  ...
  \vspace{20pt}
  \\Bestimmung der Ableitungen:	$f′(x) =  2e^{2x} - e^{-x}$, $f′′(x) =  4e^{2x} + e^{-x}$	(immer größer Null)\\
	Notwendige Bedingung für einen Extrempunkt:\\  
			$f′(x) = 0	\Leftrightarrow  2e^{2x} - e^{-x} = 0 |\cdot e^x  	\Leftrightarrow 2e^{3x} – 1 = 0  \Leftrightarrow     2e^{3x} = 1
